...
internal fun ButtonDemo() = Stack {
  ...
  Line {
    // Slot form: the content is a RowScope, so an icon is just an Icon before the Text. The
    // button publishes its icon size ambiently, so the Icon needs no size of its own.
    Button(onClick = {}) {
